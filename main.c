...
int main(int argc, char* argv[]){
    ...
    printf("Começando a ler o arquivo\n");
    while(feof(fileIn) == 0){
        ...
        for(i=0;i<numeroDeGeracoes;i++){
            ...
            if(i%1000==0)
            printf("O caminho mais curto em %d \n Geração:%f",i,calcularCusto(numeroDeCidades,geracao[0], matrizDeDistancia));
        ...
        fclose(fileOUT);
        printf("A melhor solução de custo %f é:\n",calcularCusto(numeroDeCidades,geracao[0], matrizDeDistancia));
        printf("[ "); /*escrevendo a melhor solução*/
...
/* uma função que retorna um número aleatório no intervalo <0;-1> */
int desenhar(int intervalo){
    return(rand()%intervalo);
}
/* uma função que verifica se um gene repetitivo (número da cidade) não é desenhado no cromossomo */
int checkRepeticoes(int *cromossomo, int nn, int numero){
    int i;
...
/* função que retorna o ponteiro para a matriz de soluções desenhadas 
 números (ordem das cidades visitadas) não podem ser repetidos */
int *aleatorio(int numeroOf){
    int *nDesenhados;
    int sorteio;
    int numero;
    ...
    nDesenhados = (int*)malloc((sizeof(int))*numeroOf); /*alocação de memória para tabelas numéricas */
    do{ /* desenhando os próximos números e verificando se não há duplicatas*/
...
/* a função geradora da primeira geração de soluções - pais */
void criaPais(int **pais, int numeroDeCidades, int numeroPais){
    int i;
    srand(time(NULL)); /*inicialização do embrião para a função rand*/
    for(i=0; i<numeroPais; i++){
        pais[i] =aleatorio(numeroDeCidades); /*e o pai aponta para a solução desenhada*/
    }
}
/* Função calculando o custo da viagem entre cidades individuais 
...
/* função de seleção de soluções (cromossomos) de acordo com seus custos */
...
/* função de propagação, operando com base no cruzamento */
void propagacao (int **pais, int **geracao, int numeroPais, int tamanhoGene){
    int i,j,k,temp;
    //int l;
    ...
    for(i=0;i<tamanhoGene;i++) /*o primeiro indivíduo de uma nova geração*/
        (geracao[0])[i] = (pais[0])[i]; /*torna-se o melhor indivíduo da velha geração*/
    for(i=1;i<numeroPais;i++){ /*o resto da geração é criado pelo cruzamento*/
        visitados = 0;
        geracao[i]=(int*)malloc(sizeof(int)*tamanhoGene);
        k=(rand()%(numeroPais/2)); /*k é um número aleatório no intervalo <0; (número de nascimentos / 2) -1> para cruzar apenas indivíduos melhores do conjunto pai*/
        /*l=((rand()%2)+3); //l é um número aleatório*/
...
/*função que faz uma mutação*/
void mutacao (int **geracao,int comprimentoGene ,int populacao){
    int a, b, c, j, temp, los;
    srand(time(NULL));
    for(j=0;j<populacao;j++){
        los = rand()%2;
        if(los==1){/*a probabilidade de mutação cromossômica é de 50%*/
            a = (rand()%(populacao-1)+1); /*desenhando um número da faixa <1; população-1>*/
            b = rand()%comprimentoGene; /*o melhor indivíduo não será mutado!*/
...
/*uma função que prescreve dados de um array bidimensional A para um array bidimensional B*/
void novo (int **A, int **B, int nLinhas, int nColunas){
    int i,j;
    for(i=0;i<nLinhas;i++){
